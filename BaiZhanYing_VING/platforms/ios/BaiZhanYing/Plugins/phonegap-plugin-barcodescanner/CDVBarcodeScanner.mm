...
- (void)scan:(CDVInvokedUrlCommand*)command {
    ...
    if (capabilityError) {
        ...
        NSString * error = NSLocalizedString(@"打开相机失败,请在设置中允许“百战赢+”使用相机功能。",nil);
      ...
      //                 initWithBarButtonSystemItem:UIBarButtonSystemItemCancel
        //               target:(id)self
          //             action:@selector(cancelButtonPressed:)
            //           ];
    id cancelButton = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:(id)self action:@selector(cancelButtonPressed:)];
